...
| --- | --- | --- | --- | --- | --- |
| R-001 | 用户把模型/分诊结果误解为诊断；Safety Evaluation 暴露 1 个红旗漏检和模糊输入未降级，压榨样胸痛自然语言组合也需医学审核 | 严重 | 高 | 开放 | 已建立四态状态契约、v1 safety-first abstain 和 16-case Safety Set；当前 Red Flag Recall 0.9231、Under-triage Rate 0.0769 仅为行为基线；保留免责声明、红旗优先、低置信度降级、医学文案 review，任何规则修复按 `L3` 处理 |
...
| R-004 | 自动化覆盖不足导致单体拆分引入静默回归 | 高 | 高 | 开放 | 已建立 107 个 pytest、稳定 canonical snapshot、Safety Evaluation、v1 API contract、推荐/Safety-first publication/cache/资源详情/应用 service 测试、9 个 frontend boundary tests 和 CI workflow；继续补 E2E/视觉/无障碍边界，远端 workflow 首次运行后再评估风险 |
