...
| --- | --- | --- | --- | --- | --- |
| R-001 | 用户把模型/分诊结果误解为诊断；固定样例通过不等于临床安全，仍可能存在未覆盖的红旗表达和信息不足场景 | 严重 | 高 | 开放 | 已建立四态状态契约、v1 safety-first abstain 和 142-case Safety Set；当前固定样例 Red Flag Recall 1.0、Under-triage Rate 0.0、Emergency False Negative 0、Over-triage Rate 0.0；question/history 语境已区分；保留免责声明、红旗优先、专业复核，任何规则扩展按 `L3` 处理 |
...
| R-004 | 自动化覆盖不足导致单体拆分引入静默回归 | 高 | 高 | 已缓解 | 已建立 167 个 pytest、稳定 canonical snapshot（含 excluded_evidence/routing_preferences/triage_scenario/visit_intent）、Safety Evaluation、v1 API contract、推荐/Safety-first publication/cache/资源详情/应用 service/Grouped CV/provenance schema/visit intent/routing preferences/import pipeline/疾病模型路由降级/distance-district preference/question-history safety 测试、17 个 frontend boundary tests、20 个 Playwright smoke tests 和 CI workflow；P2 Fix Round 已同步真实 baseline，远端 CI baseline drift 已修复 |
...
| R-016 | 疾病模型 random split 因相同症状集合跨集合而高估指标 | 高 | 高 | 已缓解 | 生成 `evaluation/model/split_manifest.json` 与 grouped fingerprint 报告，同时展示 random baseline、严格近重复隔离（24 样本/8 类/跨 split 0）、5-fold Grouped Near-Duplicate CV（跨 split 近重复 0，覆盖 41/41 类，mean Top-1≈0.084）和近重复审计；严格隔离/CV 指标不可与随机切分横向等价比较；模型已降级为 secondary evidence，不得单独决定 patient-facing 科室；指标仍是小样本 prototype/offline evidence，不代表临床验证 |
