...
| R-004 | 自动化覆盖不足导致单体拆分引入静默回归 | 高 | 高 | 开放 | 已建立 83 个 pytest、稳定快照、Safety Evaluation、推荐/Safety-first publication/API contract/cache 纯函数测试和 CI workflow；继续补 API/纯函数边界，远端 workflow 首次运行后再评估风险 |
