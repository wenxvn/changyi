...
```json
{
  "data": {
    ...
    "metrics": {
      "hospitals": {"value": 21, "label": "医疗机构", "source_class": "legacy_catalog_pending_provenance"},
...
- `hospitals` 当前来自 `app.py:HOSPITALS`，其 `source_class` 必须保持 `legacy_catalog_pending_provenance`，不得在 UI 中标成官方已验证目录。
